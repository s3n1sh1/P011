...
class CreateTBLMNUTable extends weMigrations
{
    ...
    public function up()
    {
        Schema::create('TBLMNU', function ($table) {
            ...
            $table->integer('TMCOMSIY')->nullable(false)->comment('System Company IY');
            ...
            $table->primary(['TMCOMSIY','TMMENUIY']);  
            $table->unique(array('TMCOMSIY','TMNOMR'));           
            $table->foreign('TMCOMSIY')->references('SCCOMSIY')->on('SYSCOM');
